...
### 4. BERT fine-tuning
#### 4.1 GLUE 实验
首先我们用 $C \in \mathbb{R}^H$ 来表示 `[CLS]` 的最终向量表示。那么对于 GLUE 任务，在 Fine-Tuning 阶段，只需要引入一个分类层就可以了，这个层里包含可训练参数 $W \in \mathbb{R}^{K\times H}$
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTEwMzM4MTkwMTksLTMzMTk1MTUwMiw1Nj
